...
public class HttpUtil {
    ...
    public static List<Hq> getAsObj(QueryType queryType,List<String> code) {

        String url = HangqingSinaServiceImpl.getUrl(queryType, code);
        if (logger.isDebugEnabled()) {
            logger.debug("url:"+url);
        }
        String rep = get(url);
        List<Hq> hqs = HangqingSinaServiceImpl.repToObj(queryType, rep);
        return hqs;
    }
}
